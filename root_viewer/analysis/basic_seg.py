...
def widget_wrapper():
    ...
    def run_cellpose(
        ...
        print(
            f"Runnint model type: {model_type}\nThis will take a couple of minutes..."
        )
    ...
    def compute_diameter(image, channels, model_type):
        ...
        try:
            ...
            Notification(f"error computing diameter: {e}")
            return 0

    @thread_worker
    def compute_masks(
        masks_orig, flows_orig, cellprob_threshold, model_match_threshold
    ):
        import cv2
    ...
    @magicgui(
        label_head=dict(widget_type="Label", label=f"<h1>Basic Segmentation</h1>"),
        call_button="run segmentation",
        layout="vertical",
        model_type=dict(
            widget_type="ComboBox",
            label="model type",
            choices=["cyto", "nuclei", "cyto2", "custom"],
            value="cyto",
    ...
    def widget(
        ...
        cp_worker = run_cellpose(
            ...
            stitch_threshold=float(stitch_threshold_3D)
    ...
    widget.label_head.value = (
        "<small>A generalist algorithm for cellular segmentation</small>"
    )
    ...
    def _report_diameter(diam):
        ...
        Notification(f"computed diameter = {diam}")
    ...
    def _compute_diameter(e: Any):
        try:
            ...
            if widget.model_type.value == "custom":
                Notification("cannot compute diameter for custom model")
            else:
    ...
    def _compute_diameter_shape(e: Any):
        try:
            ...
            if k > 0:
                ...
                Notification("no square or circle shapes created")
        except Exception as e:
            Notification(f"Error: {e}")
