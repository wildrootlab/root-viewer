...
def widget_wrapper():
    ...
    def run_cellpose(image, model_type, custom_model, channels, channel_axis, diameter,
        ...
        if model_match_threshold==0.0:
            flow_threshold = 0.0
            Notification('flow_threshold=0 => no masks thrown out due to model mismatch')
        Notification(f'computing masks with cellprob_threshold={cellprob_threshold}, flow_threshold={flow_threshold}')
        if model_type=='custom':
            CP = models.CellposeModel(pretrained_model=custom_model, gpu=True)
        else:
            CP = models.CellposeModel(model_type=model_type, gpu=True)
        print(f'Runnint model type: {model_type}\nThis will take a couple of minutes...')
