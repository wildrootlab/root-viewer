...
from magicgui.widgets import (Container, FunctionGui, Label, RadioButtons,
                              TextEdit)
...
class AnalysisWidget:
    def __init__(self):
        ...
        (
            self.filter_functions_list,
            self.filter_functions_dict,
        ) = self._create_functions_attr(filter_functions)
        (
            self.thresholding_functions_list,
            self.thresholding_functions_dict,
        ) = self._create_functions_attr(thresholding_functions)
        (
            self.labeling_functions_list,
            self.labeling_functions_dict,
        ) = self._create_functions_attr(labeling_functions)
        (
            self.misc_functions_list,
            self.misc_functions_dict,
        ) = self._create_functions_attr(misc_functions)
    ...
    def _init_filtering(self):
        ...
        self.filtering_widget = FunctionsWidget(
            self.filter_functions_list, self.filter_functions_dict
        )
    ...
    def _init_thresholding(self):
        ...
        self.thresholding_widget = FunctionsWidget(
            self.thresholding_functions_list, self.thresholding_functions_dict
        )
    ...
    def _init_labeling(self):
        ...
        self.labeling_widget = FunctionsWidget(
            self.labeling_functions_list, self.labeling_functions_dict
        )
    ...
    def _init_misc(self):
        ...
        self.misc_widget = FunctionsWidget(
            self.misc_functions_list, self.misc_functions_dict
        )
...
class FunctionsWidget:
    ...
    def _init_layout(self):
        self.layout = Container(
            widgets=[self.function, self.function_doc], scrollable=True
        )
    ...
    def set_function_doc(self, docs, doc):
        self.function_doc.value = self.format_function_doc(
            self.functions_dict[self.functions[0]].__doc__
        )
    ...
    def format_function_doc(self, doc):
        ...
        file_name = os.path.splitext(os.path.basename(inspect.getmodule(doc).__file__))[
            0
        ]
    ...
    def _format_function_doc(self, doc):
        ...
        plist = [
            ptemp.format(p.arg_name, p.type_name, p.description) for p in ds.params
        ]
        params = textwrap.wrap(
            f'<h3>Parameters</h3><ul>{"".join(plist)}</ul>', width=30
        )
        short = textwrap.wrap(
            f"<p>{ds.short_description}</p>" if ds.short_description else "", width=30
        )
        long = textwrap.wrap(
            f"<p>{ds.long_description}</p>" if ds.long_description else "", width=30
        )
